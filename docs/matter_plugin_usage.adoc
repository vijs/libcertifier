...
Once the Commissionee application is running, launch an interactive `certifier-tool` shell:

----
./certifier-tool interactive start --paa-trust-store-path ../matter_sdk/credentials/development/paa-root-certs
----

Then, to comission a device there are two alternatives shown in the subsequent subsections.

X.509 authentication:

----
pairing onnetwork-certifier 0x12 20202021 --input-p12-path dac-commissioner.p12 --input-p12-password changeit
----

SAT authentication:

----
pairing onnetwork-certifier 0x12 20202021 --sat 1 --sat-token <token>
----
...
----
pairing onnetwork-certifier 0x12 20202021 --paa-trust-store-path ../matter_sdk/credentials/production/paa-root-certs --sat 1
----
