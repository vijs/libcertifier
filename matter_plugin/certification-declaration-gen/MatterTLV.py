...
#
#   @file
#         This file contains definitions for working with data encoded in Matter TLV format
#
...
def extractProfile(val):
    return (val & 0xffff)

def extractVendor(val):
    return (val >> 16) & 0xffff

def makeProfileTag(vendor, profile, tagNum):
    return (((vendor & 0xffff) << 16) | (profile & 0xffff), tagNum)
    ...
    def _encodeControlAndTag(self, type, tag, lenOfLenOrVal=0):
        ...
        if isinstance(tag, tuple):
            ...
            if profile is None or profile == self._implicitProfile:
                ...
                vendor = extractVendor(profile)
                profileNum = extractProfile(profile)
                if tagNum <= UINT16_MAX:
                    controlByte |= TLV_TAG_CONTROL_FULLY_QUALIFIED_6Bytes
                    return struct.pack("<BHHH", controlByte, vendor, profileNum, tagNum)
                else:
                    controlByte |= TLV_TAG_CONTROL_FULLY_QUALIFIED_8Bytes
                    return struct.pack("<BHHL", controlByte, vendor, profileNum, tagNum)
        raise ValueError("Invalid object given for TLV tag")
...
class TLVReader(object):
    ...
    def _decodeControlAndTag(self, tlv, decoding):
        ...
        if decoding["tagControl"] == "Anonymous":
            ...
            (vendor_id, profile_num) = struct.unpack("<HH", tlv[self._bytesRead : self._bytesRead + 4])
            profile = (vendor_id << 16) | (profile_num)
